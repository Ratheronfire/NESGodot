...
class InstructionData:
    ...
    func _write_value_to_memory(value, implied_register = Consts.CPU_Registers.A):
        if context.address_mode == Consts.AddressingModes.Accumulator:
            ...
            var address = _determine_memory_address()
            NES.cpu_memory.write_byte(address, value)

    func _read_value_from_memory(implied_register = Consts.CPU_Registers.A):
    ...
    func _determine_memory_address():
    ...
    func execute():
        match self.instruction:
            'LDA':
                var value = _read_value_from_memory()
                NES.cpu_memory.registers[Consts.CPU_Registers.A] = value
            ...
            'STA':
                _write_value_to_memory(NES.cpu_memory.registers[Consts.CPU_Registers.A])
            'STX':
                _write_value_to_memory(NES.cpu_memory.registers[Consts.CPU_Registers.X])
            'STY':
                _write_value_to_memory(NES.cpu_memory.registers[Consts.CPU_Registers.Y])
            'TAX':
            ...
            'TXS':
                _transfer(Consts.CPU_Registers.X, Consts.CPU_Registers.SP, true)
            'ADC':
                var a = NES.cpu_memory.registers[Consts.CPU_Registers.A]
                var new_a = a
                ...
                var added_value = _read_value_from_memory()
                new_a += added_value
            ...
            'SBC':
                ...
                var subtracted_value = _read_value_from_memory()
            ...
            'ASL':
                ...
                _write_value_to_memory(shifted_value)
            'LSR':
                var value = _read_value_from_memory()
            ...
            'ROR':
                ...
                _write_value_to_memory(shifted_value)
            'PHA':
                Opcodes.push_to_stack(NES.cpu_memory.registers[Consts.CPU_Registers.A])
            'PLA':
                NES.cpu_memory.registers[Consts.CPU_Registers.A] = Opcodes.pull_from_stack()
            ...
            'PHP':
                Opcodes.push_to_stack(NES.cpu_memory.registers[Consts.CPU_Registers.P])
            ...
            'PLP':
                NES.cpu_memory.registers[Consts.CPU_Registers.P] = Opcodes.pull_from_stack() & 0b11101111
            'JMP':
                ...
                var address = context.value if context.address_mode == Consts.AddressingModes.Absolute else _determine_memory_address()
            ...
            'JSR':
                ...
                Opcodes.push_to_stack(pc >> 8)
                Opcodes.push_to_stack(pc & 0xFF)
            ...
            'RTS':
                ...
                NES.cpu_memory.registers[Consts.CPU_Registers.PC] = Opcodes.pull_from_stack() + (Opcodes.pull_from_stack() << 8) + 1
            ...
            'RTI':
                ...
                NES.cpu_memory.registers[Consts.CPU_Registers.P] = Opcodes.pull_from_stack()
                NES.cpu_memory.registers[Consts.CPU_Registers.PC] = Opcodes.pull_from_stack() + (Opcodes.pull_from_stack() << 8)
